...
生成时间（UTC）：2026-08-01T12:57:20Z。计算脚本：`work/i1_compute_underflow_regimes.py`；报告脚本：`work/i1_make_report.py`。
...
- 区间②在样本中集中于 signature N 层：Mac 为 145/500（29.0%），HPC 为 143/500（28.6%）。D0-D4 的既有分母口径 `zero_postwarm_scope == True` 整体排除了该层，因此原 1,800 行分母数字只能解释为 U/Z 域内占比。
- U/Z 域内区间②设计基估计：Mac 合并 1.417% [0.603%, 2.442%]，HPC 合并 0.953% [0.214%, 1.867%]；这不是全域或全语料占比。
- `nzero==6` 风险格在 N 层 Mac/HPC 各 7/500（1.40%），且 7 行全为 waymo；这是由下溢删掉六个候选后形成的事实 hard argmax，现有 legacy 标记不会单独提示。
- 新增辅助全域口径 U∪Z∪N 只在 Mac 侧有设计基估计：区间②合并 25.752% [21.346%, 30.210%]。这个分母是 D0-D4 从未使用过的口径，不能与 D0-D4 既有结论直接并列；HPC 侧 N 层 `ht_weight` 全为 1.0 占位符，不能给全域设计基估计。
...
**口径订正说明：** 以下第一组既有数字的分母固定为 `zero_postwarm_scope == True`；本轮增量脚本断言该条件等价于 `signature ∈ {U,Z}`。因此每版 1,800 行的既有表格是 **U/Z 域内（zero_postwarm_scope==True，等价于 signature∈{U,Z}）** 估计，整体排除 500 个 `signature N` 锚点。N 恰是区间②的主要发生层，不能把这些旧数字读成全域或全语料占比。

U/Z 域内（zero_postwarm_scope==True，等价于 signature∈{U,Z}）每版 1,800 行，cluster 为 `scene_unique_id`，B=2000，seed=20260731，percentile CI。若样本命中数为 0 或 1，CI 标为不可给出。

| 版本 | U/Z 域内分母行 | U/Z 域内cluster数 | U/Z 域内HT权重和 | U/Z 域内source行数 | U/Z 域内source权重和 |
| --- | --- | --- | --- | --- | --- |
...
| 指标（U/Z 域内） | 版本 | source/合并（U/Z 域内） | U/Z 域内样本行 | U/Z 域内样本命中 | U/Z 域内cluster | U/Z 域内HT命中权重/HT分母 | U/Z 域内HT设计基估计占比（95% CI） |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 区间②（U/Z 域内设计基估计） | Mac | all | 1800 | 19 | 1459 | 7,579.5 / 534,939 | 1.417% [0.603%, 2.442%] |
| 区间②（U/Z 域内设计基估计） | Mac | nuplan | 900 | 8 | 632 | 227.84 / 145,821 | 0.156% [0.039%, 0.304%] |
| 区间②（U/Z 域内设计基估计） | Mac | waymo | 900 | 11 | 827 | 7,351.66 / 389,118 | 1.889% [0.779%, 3.355%] |
| 区间②（U/Z 域内设计基估计） | HPC | all | 1800 | 7 | 1459 | 5,095.65 / 534,939 | 0.953% [0.214%, 1.867%] |
| 区间②（U/Z 域内设计基估计） | HPC | nuplan | 900 | 0 | 632 | 0 / 145,821 | 0.000%；CI 不可给出（insufficient_hits） |
| 区间②（U/Z 域内设计基估计） | HPC | waymo | 900 | 7 | 827 | 5,095.65 / 389,118 | 1.310% [0.298%, 2.532%] |
| 区间③，第0节定义（U/Z 域内设计基估计） | Mac | all | 1800 | 3 | 1459 | 2,045.55 / 534,939 | 0.382% [0.000%, 0.967%] |
| 区间③，第0节定义（U/Z 域内设计基估计） | Mac | nuplan | 900 | 1 | 632 | 55.0333 / 145,821 | 0.038%；CI 不可给出（insufficient_hits） |
| 区间③，第0节定义（U/Z 域内设计基估计） | Mac | waymo | 900 | 2 | 827 | 1,990.52 / 389,118 | 0.512% [0.000%, 1.286%] |
| 区间③，第0节定义（U/Z 域内设计基估计） | HPC | all | 1800 | 3 | 1459 | 2,045.55 / 534,939 | 0.382% [0.000%, 0.967%] |
| 区间③，第0节定义（U/Z 域内设计基估计） | HPC | nuplan | 900 | 1 | 632 | 55.0333 / 145,821 | 0.038%；CI 不可给出（insufficient_hits） |
| 区间③，第0节定义（U/Z 域内设计基估计） | HPC | waymo | 900 | 2 | 827 | 1,990.52 / 389,118 | 0.512% [0.000%, 1.286%] |
| 区间③可见差异子集，补充（U/Z 域内设计基估计） | Mac | all | 1800 | 2 | 1459 | 1,990.52 / 534,939 | 0.372% [0.000%, 0.952%] |
| 区间③可见差异子集，补充（U/Z 域内设计基估计） | Mac | nuplan | 900 | 0 | 632 | 0 / 145,821 | 0.000%；CI 不可给出（insufficient_hits） |
| 区间③可见差异子集，补充（U/Z 域内设计基估计） | Mac | waymo | 900 | 2 | 827 | 1,990.52 / 389,118 | 0.512% [0.000%, 1.286%] |
| 区间③可见差异子集，补充（U/Z 域内设计基估计） | HPC | all | 1800 | 2 | 1459 | 1,990.52 / 534,939 | 0.372% [0.000%, 0.952%] |
| 区间③可见差异子集，补充（U/Z 域内设计基估计） | HPC | nuplan | 900 | 0 | 632 | 0 / 145,821 | 0.000%；CI 不可给出（insufficient_hits） |
| 区间③可见差异子集，补充（U/Z 域内设计基估计） | HPC | waymo | 900 | 2 | 827 | 1,990.52 / 389,118 | 0.512% [0.000%, 1.286%] |
| nzero==6，事实hard argmax风险格（U/Z 域内设计基估计） | Mac | all | 1800 | 10 | 1459 | 4,317.87 / 534,939 | 0.807% [0.179%, 1.607%] |
| nzero==6，事实hard argmax风险格（U/Z 域内设计基估计） | Mac | nuplan | 900 | 4 | 632 | 71.3333 / 145,821 | 0.049% [0.003%, 0.135%] |
| nzero==6，事实hard argmax风险格（U/Z 域内设计基估计） | Mac | waymo | 900 | 6 | 827 | 4,246.53 / 389,118 | 1.091% [0.244%, 2.189%] |
| nzero==6，事实hard argmax风险格（U/Z 域内设计基估计） | HPC | all | 1800 | 2 | 1459 | 1,990.52 / 534,939 | 0.372% [0.000%, 0.954%] |
| nzero==6，事实hard argmax风险格（U/Z 域内设计基估计） | HPC | nuplan | 900 | 0 | 632 | 0 / 145,821 | 0.000%；CI 不可给出（insufficient_hits） |
| nzero==6，事实hard argmax风险格（U/Z 域内设计基估计） | HPC | waymo | 900 | 2 | 827 | 1,990.52 / 389,118 | 0.512% [0.000%, 1.325%] |

按 signature 的补充设计基估计（U/Z 域内，只有 U 与 Z；N 被上述分母整体排除，只列区间②与 `nzero==6`）：

| 指标（U/Z 域内） | 版本 | signature（U/Z 域内） | U/Z 域内样本命中 | U/Z 域内HT设计基估计占比（95% CI） |
| --- | --- | --- | --- | --- |
| 区间②（U/Z 域内设计基估计） | Mac | U | 10 | 0.979% [0.078%, 2.122%] |
| 区间②（U/Z 域内设计基估计） | Mac | Z | 9 | 1.798% [0.418%, 3.542%] |
| 区间②（U/Z 域内设计基估计） | HPC | U | 0 | 0.000%；CI 不可给出（insufficient_hits） |
| 区间②（U/Z 域内设计基估计） | HPC | Z | 7 | 1.782% [0.401%, 3.529%] |
| nzero==6（U/Z 域内设计基估计） | Mac | U | 8 | 0.935% [0.036%, 2.068%] |
| nzero==6（U/Z 域内设计基估计） | Mac | Z | 2 | 0.696% [0.000%, 1.808%] |
| nzero==6（U/Z 域内设计基估计） | HPC | U | 0 | 0.000%；CI 不可给出（insufficient_hits） |
| nzero==6（U/Z 域内设计基估计） | HPC | Z | 2 | 0.696% [0.000%, 1.808%] |

### signature 分层样本计数

这张表是样本内计数，不是设计基估计。它显示区间②主要集中在被 U/Z 域分母排除的 N 层。

| signature | 锚点数 | 区间② Mac | 区间② HPC | nzero==6 Mac | nzero==6 HPC |
| --- | --- | --- | --- | --- | --- |
| N | 500 | 145（29.0%） | 143（28.6%） | 7（1.40%） | 7（1.40%） |
| U | 1200 | 10（0.8%） | 0（0.0%） | 8（0.67%） | 0（0.00%） |
| Z | 600 | 9（1.5%） | 7（1.2%） | 2（0.33%） | 2（0.33%） |

### signature N 层估计

N 层每版 500 行。Mac 的 `ht_weight` 是 4 个真实设计权重（2 source × 2 n_band 单元），本节按同一 HT 比率估计 + cluster bootstrap 程序计算；N 层内 cluster 为 491 个 `scene_unique_id`，B=2000，seed=20260731。HPC 的 N 层 `ht_weight` 全为 1.0，占位符不是设计权重，因此 HPC 侧 N 层只报未加权样本占比，不给 HT 加权估计。

| 版本 | domain | 样本行 | cluster | HT权重和 | source行数 | source权重和 | 权重状态 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Mac | N | 500 | 491 | 2,111,119 | {"nuplan": 250, "waymo": 250} | {"nuplan": 1162747.0, "waymo": 948372.0} | mac_design_weight |
| HPC | N | 500 | 491 | 500 | {"nuplan": 250, "waymo": 250} | {"nuplan": 250.0, "waymo": 250.0} | hpc_placeholder_weight_all_1 |
...
| 区间② | HPC | all | 143/500（28.60%） | 491 | hpc_placeholder_weight_all_1 | 未加权样本占比 28.600%；设计基估计不可给出（hpc_placeholder_weight_all_1） |
| 区间② | HPC | nuplan | 47/250（18.80%） | 246 | hpc_placeholder_weight_all_1 | 未加权样本占比 18.800%；设计基估计不可给出（hpc_placeholder_weight_all_1） |
| 区间② | HPC | waymo | 96/250（38.40%） | 245 | hpc_placeholder_weight_all_1 | 未加权样本占比 38.400%；设计基估计不可给出（hpc_placeholder_weight_all_1） |
...
| 区间③（第0节定义） | HPC | all | 6/500（1.20%） | 491 | hpc_placeholder_weight_all_1 | 未加权样本占比 1.200%；设计基估计不可给出（hpc_placeholder_weight_all_1） |
| 区间③（第0节定义） | HPC | nuplan | 4/250（1.60%） | 246 | hpc_placeholder_weight_all_1 | 未加权样本占比 1.600%；设计基估计不可给出（hpc_placeholder_weight_all_1） |
| 区间③（第0节定义） | HPC | waymo | 2/250（0.80%） | 245 | hpc_placeholder_weight_all_1 | 未加权样本占比 0.800%；设计基估计不可给出（hpc_placeholder_weight_all_1） |
...
| nzero==6，事实hard argmax风险格 | HPC | all | 7/500（1.40%） | 491 | hpc_placeholder_weight_all_1 | 未加权样本占比 1.400%；设计基估计不可给出（hpc_placeholder_weight_all_1） |
| nzero==6，事实hard argmax风险格 | HPC | nuplan | 0/250（0.00%） | 246 | hpc_placeholder_weight_all_1 | 未加权样本占比 0.000%；设计基估计不可给出（hpc_placeholder_weight_all_1） |
| nzero==6，事实hard argmax风险格 | HPC | waymo | 7/250（2.80%） | 245 | hpc_placeholder_weight_all_1 | 未加权样本占比 2.800%；设计基估计不可给出（hpc_placeholder_weight_all_1） |

### U∪Z∪N 全域辅助口径

以下全域分母覆盖全部 2,300 个锚点，cluster 为全部 1,909 个 `scene_unique_id`，B=2000，seed=20260731。这个分母是 D0-D4 从未使用过的新增辅助口径，只用于理解 N 层被排除后的量级变化，不能与 D0-D4 的既有结论直接并列。HPC 侧因 N 层权重是全 1.0 占位符，无法给出 U∪Z∪N 全域设计基估计；对应未加权诊断计数只保存在增量 CSV 中。

| 版本 | domain | 样本行 | cluster | HT权重和 | source行数 | source权重和 | 权重状态 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Mac | U_Z_N | 2300 | 1909 | 2,646,058 | {"nuplan": 1150, "waymo": 1150} | {"nuplan": 1308568.0, "waymo": 1337490.0} | mac_design_weight |
| HPC | U_Z_N | 2300 | 1909 | 535,439 | {"nuplan": 1150, "waymo": 1150} | {"nuplan": 146071.0, "waymo": 389368.0} | hpc_N_placeholder_weight_all_1 |
...
N 层两版高度一致：区间②样本占比分别为 Mac 145/500（29.0%）与 HPC 143/500（28.6%），`nzero==6` 完全相同，均为 7/500（1.40%），且 7 行全为 waymo。U 层是主要分歧位置：Mac 区间② 10/1,200，HPC 为 0/1,200；`nzero==6` 也为 Mac 8/1,200、HPC 0/1,200。Z 层差异较小：区间② Mac 9/600、HPC 7/600，`nzero==6` 两版均为 2/600。因此本轮结论应写成“分歧是 U 层现象，N 层两环境高度一致”，而不是笼统写成加权后差距缩小或 CI 重叠。
