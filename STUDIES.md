...
| RQ014 | WOD-E2E lost rating↔IPV-deviation result recovery | v1.5 FORMAL_G1_PASS after sixth-round dual review; 960-cell historical-recovery grid frozen; no empirical run yet; first staged operation is rating-free declassification export and still requires published exact commit + validate-only | `reports/studies/RQ014_wod_e2e_rating_recovery/` | `reports/knowledge/RQ014_wod_e2e_rating_recovery/` | no manuscript claim; same-dataset recovery requires 2,880-row terminal screen + clean replay after separate gates |
