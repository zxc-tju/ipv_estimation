...
| RQ015A | **IPV 估计尝试状态与候选权重集中度回溯审计**（v3 计划 + **schema v2**；continuous `q_eff` primary；bins 不进入 episode/C0） | building / PI 裁定按**路径 A**（先写完实现再送最后一轮复审）；预执行合同核验完成得 **C1–C14**（含 C6 fold≠split 安全阻断、C3 产物指认错、C1 恒等式基数错、C14 WOD 三产物本地缺失）；fixtures 20/20；`execution_authorized=false`、`formal_g1_eligible=false` | `reports/studies/RQ015A_ipv_estimability_labelling/` | `reports/knowledge/RQ015A_ipv_estimability_labelling/` | 交接手册 `reports/plans/RQ015A_execution_handoff_20260727.md`；可审计范围 3/6 产物（InterHub sigma01 + RQ009 feature matrix + OnSite），WOD 一支待只读取回；不得声称测出/未测出 IPV |
