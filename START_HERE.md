...
- **【2026-09-04 RQ029 模板包已重建并通过 74/74 校验。】** 当前可写入模板入口为 `data/derived/rq029_human_template/v1/`：20 drivers、300 runs、81,880 frames；包内除根 `README.md` 外无 `synthetic` 路径或内容命中；`placeholder_registry.csv`、`required_real_collection_fields.csv` 与 `import_contract.json` 已明确列出待补充的真实采集信息；`validate_release --mode template` 为 `PASS`，`python3 -m pytest tests/test_rq029_human_template.py tests/test_rq029_layered_release.py -q` 为 `11 passed`。**硬边界**：这是模板壳，不是实测数据；真实导入前必须替换或确认 `taskId`、`recordId`、`vehicle`、逐帧日志和场景映射。**
