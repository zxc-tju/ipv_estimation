#!/bin/bash
#SBATCH --job-name=ipv_s01_full
#SBATCH --comment="Full InterHub IPV rerun with agent.sigma=0.1, 4 nodes x 96 CPUs"
...
EXTRA_ARGS=()
MODE="all fullset rows"
if [ "$ONLY_INCOMPLETE" = "1" ]; then
    EXTRA_ARGS+=(--only-incomplete)
    MODE="incomplete fullset rows only"
fi
