...
## 8. `run_receipt.json` 顶层 measurement-field 标志误标（2026-07-31 A2 追加）

本轮 A1 execute 确实读取了 measurement 字段，但最终 `run_receipt.json` 顶层
`reads_measurement_fields = false` 是从 validate 路径继承来的误标。真值已记录在同一回执的
`metadata.execute_measurement_fields_read = true`，因此这不是隐瞒，也不影响本轮 bounded report
对读数边界的披露。

监督方已查明契约本身不强制该字段为 false：`receipt.py:134` 只要求 bool。真实 execute 路径中
`run_rq015a.py:301` 应写 true；`run_rq015a.py:1146` 属 permit-DENIED 路径，未构造 reader，保持
false 是正确的。本轮按速度原则不修，因为修正会连带测试、v10 到 v11 重签与 14,473,982 行重跑。

下一次正当触碰该脚本时的修法：区分 `run_rq015a.py:301`（应为 true）与
`run_rq015a.py:1146`（应保持 false），并同步修改
`tests/test_rq015a_run_entrypoint.py:222`。

## 9. `sigma01` 存档主表的逐锚点值在当前 HEAD 上未能复现（2026-07-31 track B 追加）

RQ015B 本轮（分层抽样 2,300 锚点，split 仅 development/guard）在解算前设了一道复现门：
取存档主表 `sigma01_ipv_timeseries.csv`（产出于 **2026-06-12**）中的锚点，
用**当前 HEAD** 的估计器重解，比对逐锚点结果。**该门未过。**

```
gate_a（逐锚点值复现）  12/40   阈值 >= 39/40   ✗
gate_b（本轮内部一致性） 40/40                  ✓
```

分层结构比合计数更重要：

- **U 层 12/14 通过，但不构成强复现证据。** U 层是均匀兜底行，而均匀兜底是不动点——
  任何落进兜底的运行都精确给出同一对值（`ipv = 0.0`、`ipv_error = 0.6220355269907728`）。
  它只支持"兜底触发可复现"，不支持"估计器数值可复现"。
- **Z 层 0/14、N 层 0/12，即真正携带数值信息的 26 个锚点全部未复现。**
  28 个失败中 15 个仅 `ipv_error` 有差（中位 3.4e-05），13 个两者皆差
  （最大 `ipv_diff` 达 `3π/8`，为整格错位）。

已排除的一个假说：leader 曾猜测差异来自"完整 pair-call 与窗口单帧调用"的口径不同，
B1 实测证伪——两种调用在失败锚点给出相同的重解结果。
剩余候选为求解器差异、参考线处理差异，以及**存档生成时的 checkout 与当前 HEAD 之间的代码漂移**；
代码漂移是最合理候选（存档距今约 7 周），但本轮按速度原则**未追查**，
也未回放历史提交（当时 track A 有 12 项未提交改动，`git checkout` 会毁掉它）。

**影响范围（这一条是本节的要点）：** 任何建立在 `sigma01` 存档主表上的、
"估计器在该数据上会产出 X"形态的主张，都需要在当前代码上重新确认后才能继续使用。
本轮自身的机制拆分比例已按此收窄口径——它刻画的是**当前代码在抽样锚点上的行为**，
**不得读作存档 M3 标签的成分构成**。

**这是留给独立一轮的事项，不是本轮的缺陷。** 追查代码漂移需要回放历史提交，
应在工作区干净、无并发 track 时单独安排。
