...
Primary corrected LOTO sample: n=53, delta Spearman=0.136833, MAE reduction=0.479264, delta CV-R2=0.086249. This primary direction is suggestive, nonsignificant (p=0.30, CI includes 0), non-generalizing (LOSO delta Spearman +0.017), and non-IPV-specific after negative controls.
...
This exploratory rerun is boundary mapping only. No interpretable exploratory stratum reached q <= 0.10, and low-n, reverse-direction, and non-FDR-stable rows require abstention. These state-dependence results do not establish robustness, do not rescue LOSO generalization, and do not show an IPV-specific incremental signal.

Settled interpretation: no robust incremental predictive utility relative to the prespecified kinematic+safety baseline was demonstrated.
