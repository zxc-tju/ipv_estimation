...
LAUNCHER_SHA256=522ca0931f8248b965ff297304f19c822534d73b9a321fba834bf53a90f55074
...
test -f "$LAUNCHER"
test ! -L "$LAUNCHER"
test "$(/usr/bin/stat -c %s "$LAUNCHER")" = 161337
test "$(digest_of "$LAUNCHER")" = "$LAUNCHER_SHA256"
test -f "$PREFLIGHT"
test ! -L "$PREFLIGHT"
test "$(/usr/bin/stat -c %s "$PREFLIGHT")" = 82031
