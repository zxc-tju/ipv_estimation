...
- **【2026-09-02 RQ029 上海 20×15 人类轨迹合成包已生成并验证 PASS，但它不是实测数据。】** 用户确认人类驾驶全部在上海后，已用上海 T11/WSD session `6923-1766197775` 的 15/15 场景构造 20 名合成驾驶员 × 15 场景 = 300 runs；每位保留 AV 同构的四项必要日志，`simulation_trajectory.log` 与源逐字节一致，感知日志全部非主车对象逐字段一致，主车仅沿源折线做平滑时间重参数化（速度绝对相对偏差 p95 `5.0615%`）。大型包入口为 `data/derived/rq029_human_synthetic_microdata/v1/`，候选/门计数为 `78,903 / 40,993 / 15,598`，最终验证 `195/195 PASS`，专项测试 `7 passed`。**硬边界**：`status`、参照带、`ipv_log` 与 `_calibrated` 签名列是按 RQ022 已知聚合边际约束合成的，frozen estimator 未重跑；真实逐行联合分布、个体效应和真实 case 映射不可恢复。本包只能用于工程联调/绘图/读取器测试，不能替代受控真实随件表、不能算 RQ022 独立复算或新手稿证据。执行报告：`reports/studies/RQ029_human_synthetic_microdata/RQ029_1_shanghai_synthetic_20260902/REPORT.md`。】**
...
- **RQ019-B1 已执行，当前 `WAITING_ON_COMMANDER`（2026-08-05T00:49:59Z），尚未经过监督方独立复算，
  不构成正式知识层结论。** 这轮承接 RQ018 的安全裕度分布压缩结果，检验 IPV 低于情境人类参照
  下界（比人类更竞争激进）时，锚点后的交互对手车是否出现更大的制动、降速或转向反应。
  分析集严格复现为第一道门 `status == OK` 且第二道门 `mechanism2_gate_ok == True` 的
  **14,099/67,861 行**，覆盖 231 个 case、19 个 team；α=90 上侧/下侧/区间内为
  2,700/1,998/9,401。结果变量只取原始 `simulation_trajectory.log` 的锚点后记录，
  `speed` 用 km/h、`acceleration` 用 m/s²，`courseAngle` 实测 0.007–359.975 后按度处理。
  **对齐三条硬断言全部通过**：有候选序列的 12,769/14,099 个锚点中最近时间差 95 分位 48 ms
  （阈值 <150 ms）；最终 `<150 ms` 匹配 12,750/14,099，未匹配拆为原始对手序列缺失
  1,330/14,099 与时间差超限 19/14,099；去除 per-case 中位平移后位置残差中位 0.0992 m
  （可比 12,750/14,099，阈值 <0.5 m）；原始 `abs(acceleration)>10` 为
  361/2,280,356 = 0.0158%（阈值 <1%）。这 361 个值含最大约 1.76e263 的明显损坏值，
  硬断言计数后在结果窗口显式设为缺失。
  **主结论只用非 scripted 组**（13,024/14,099 行，204/231 case；结果非空下侧 1,821、
  区间内 7,574）。固定 3 s 的常规降速分布中部更高：窗口速度极差中位 4.786 对 2.758 km/h，
  锚点速度减窗口最小速度中位 2.844 对 1.213 km/h；但强制动尾部更低——`a<-2` 为
  2,867/53,714 = 5.3375% 对 20,643/224,624 = 9.1900%，差 −0.0385、case bootstrap
  95% CI [−0.0605, −0.0140]；`a<-3` 为 1,521/53,714 = 2.8317% 对
  15,769/224,624 = 7.0202%，差 −0.0419、CI [−0.0627, −0.0219]；`a<-4` 为
  1,367/53,714 = 2.5450% 对 14,334/224,624 = 6.3813%，差 −0.0384、
  CI [−0.0571, −0.0220]。因此是**常规调整分布中部变化，不是极端制动恶化**。
  α=80/90/95 的降速中位与强制动占比方向一致；转向中位方向在 α=80 与 α=90/95 间改变，
  转向证据不稳定。固定 3 s 的 case 标签置换对三个强制动占比给经验 p=0.0040–0.0060；
  安慰剂曝露六个下侧阈值差的 case-bootstrap CI 均跨 0。scripted 精确标签在完整锚点表为
  7,305/67,861 行、30 case，过两道门后为 1,075/14,099 行、27 case，已隔离报告。
  **可行性试算**：5 个会话各取一个锚点，4/5 在 ±150 ms 内有周边原始记录，共 26 辆 50 m 内
  交通参与者，加速度与航向结果均为 26/26；这只说明路径可行，不做推断。
  **产物**：`.codex-fleet/rq019-counterpart-burden/work/B1/` 下可复跑脚本及 7 个 JSON，报告为
  `.codex-fleet/rq019-counterpart-burden/board/reports/RQ019_1_counterpart_burden.md`。
  **边界**：探索性关联，不使用因果措辞，不评价车辆/队伍；570,761 m 的 7 行源坐标异常进入分析集
  0/14,099；固定 3 s 窗口越出 case 末尾 734/14,099，均裁到 case 内；尚无 `decision.md`。
