...
- RQ014 focused verification command uses the existing verifier environment:
  ...
  W4f review-candidate result: `228 passed`; prior final release result: `194 passed`; broader non-shortcut regression excluding the locally absent
...
- **RQ014 W4f is a review candidate, not executable authority.** The rating-blind WOD mapping addendum freezes
  ...
  scene-level candidate contains 254 mappings (`CP=115, HO=90, MP=48, F=1`) and 225 explicit
  `UNMAPPED_EXCLUDED` scenes; missing lookup fails closed for every lane-v3 cell/readout. The v1p6 registry and
  envelope/execution contracts now checksum-bind the definition, stdlib implementation, and table, increasing
  the exact active science-binding count from 9 to 12. The existing W4d Formal G1/final bundle do not cover these
