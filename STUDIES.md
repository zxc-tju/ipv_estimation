...
| RQ017 | 在 OnSite 自动驾驶车数据上产出机制一判据，并首次量出两道门串联后的实际可用率 | **executed 2026-08-04（同济 HPC）** — 监督方已独立复算放行。67,861 个 AV 锚点帧：`OK` 37,520 = 55.2971%、`ABSTAIN` 30,341 = 44.7029%（全部 `NEAR_UNIFORM`）、`NO_IPV_EFFECT` 0、工程失败 0。与 RQ016C 支持门交叉后**两门都过 14,099 = 20.7763%**。**Case 级（分母 267）：至少 1 帧可判的 231 个 = 86.5169%；因机制一全程无解的 0 个 = 0.0000%** | `reports/studies/RQ017_onsite_mechanism_one/RQ017_1_onsite_gate_20260804T075311Z_406e7a65/` | `reports/knowledge/RQ017_onsite_mechanism_one/` | **尚无 `decision.md`**；**本轮不对任何车辆作出判断**；`NO_IPV_EFFECT` 在 OnSite 上实际不可达（0/67,861，最小非零 `mse_spread` 2.32e-08 对 InterHub 4.77e-15），弃权理由构成不可与 InterHub 对比、只能比总弃权率；机制二比的是运动学邻域非 IPV 数值，「不通过」≠「不像人」；机制二缺口是重叠不是数量（`MP` 两格逾百万人类支撑而通过率仅 13–15%，`F\|priority` 仅 45,283 行却 47.03%）；参考线用观测轨迹 fallback，与 InterHub 不同源，可比性由同一软件栈保证（G 锚点重算 `max_abs_diff = 0.0`）；未解释观察：短历史行通过率 73.92% 高于满历史 54.85%；描述性结果，禁用 estimability 表述 |
| RQ018 | 异常 IPV 与行为劣化是否存在对应关系（OnSite 正向探索） | **executed 2026-08-04（本机）** — 监督方独立复算后**改写了执行方结论**。**超出人类参照的 IPV 表达显著压缩了安全裕度的整体分布，但不制造极端危险情形**（PI 2026-08-05 定性：IPV 本就不是安全代理指标，分布压缩才是有价值的判据；「未观察到更高风险」是边界条件不是结论）：分布整体左移，中位 7.51 s 对 8.81 s（−14.8%）、75% 分位 12.75 s 对 22.38 s（−43.0%）；IPV 低于该情境人类参照下界（即比人类更激进）的帧，后续最小 TTC 中位更短（7.51 s 对 8.81 s），**但危险阈值以下的帧占比一致更低**——TTC<2 s 为 5.28%(96/1,819) 对 9.85%(861/8,739)，TTC<3 s 为 12.53%(228/1,819) 对 16.90%(1,477/8,739)；case 层 bootstrap 1,000 次 TTC<2 s 占比差 −0.0457、95% CI [−0.0696, −0.0227] 不含 0。中位数下降来自安全端长 TTC 减少而非危险端增加（25% 分位 4.105 对 4.089）。上侧越界（更合作让行）无任何劣化对应（系数 −0.0146、p=0.8564）。分析集为两门都过的 14,099 帧、231 case、19 team | `reports/studies/RQ018_abnormal_ipv_degradation/RQ018_1_association_20260804T224427Z_276cf4c/` | `reports/knowledge/RQ018_abnormal_ipv_degradation/` | **尚无 `decision.md`**；**禁用「导致」等因果表述**；**不得把「TTC 中位更短」读成「更危险」**；**IPV 符号语义**：`util = cos(ipv)×自身代价 + sin(ipv)×交互代价`，IPV 越负 = 越竞争激进，故下侧越界是「比人类更激进」不是「更消极」；下侧越界 ≠ `IPV<0`（区间内另有 38.41% 的行也是负 IPV）；主口径无结果（原始 `future_min_ttc_s` 全部不显著 p_case 0.31–0.37，`log1p` 为事后变换）；TTC 缺失与曝露相关（上侧 13.70%/下侧 8.96%/区间内 7.04%）；结果变量互相矛盾（最小距离方向相反且显著 +1.0396、p_case=0.0064）；共 288 个 p 值，标签置换 p=0.0149 承受不了多重校正；与 RQ012B 冻结结论 `RQ012-KC-HARM-NULL` 是不同曝露定义与分析单元，**不构成推翻**且方向不矛盾 |
