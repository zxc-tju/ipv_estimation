...
from sociality_estimation.planning.utility import draw_rectangle, get_central_vertices
from sociality_estimation.planning.lattice_planner import lattice_planning
...
class Simulator:
    ...
    def visualize_final_results(self, file_path, language='ENG'):
        ...
        # plot the trajectory every 5 steps
        for t in range(0, num_frame, 3):
        ...
        # ----connect two agents
        for t in range(0, self.num_step + 1, 3):
        ...
        plt.savefig(file_path + 'final.png', dpi=600)
        plt.savefig(file_path + 'final.svg')
        plt.close()
...
def main_simulate_t_intersection():
    ...
    """
    param_v_lt = np.array([0.8, 0.8])
    param_v_gs = np.array([0.7, 0.7])
    role_under_test = None
    # bg_ipv = 1 * math.pi / 8  # ipv of background vehicle
    case_id = 0

    ipv_list = [-2, -1, 0, 1, 2, 3, 4]
    simu_time = strftime("%Y-%m-%d-%H-%M", gmtime())

    for lt_ipv in tqdm(ipv_list):
        for gs_ipv in ipv_list:
            # set tag to record time and ipv
            tag = simu_time + '/lt' + str(lt_ipv) + '-gs' + str(gs_ipv)

            '---- set initial state of the left-turn vehicle ----'
            init_position_lt = [8.5, -8.5]
            init_velocity_lt = [1.0, 2.0] * param_v_lt
            init_heading_lt = math.pi / 3
            ipv_lt = math.pi / 8 * lt_ipv
            controller_type_lt = 'linear-gt'
            ...
            init_velocity_gs = [-3.0, 0.0] * param_v_gs
            init_heading_gs = math.pi
            ipv_gs = math.pi / 8 * gs_ipv
            controller_type_gs = 'linear-gt'
            ...
            # time1 = time.perf_counter()
            simu.interact(simu_step=35, iter_limit=5, make_video=False)
            # time2 = time.perf_counter()
