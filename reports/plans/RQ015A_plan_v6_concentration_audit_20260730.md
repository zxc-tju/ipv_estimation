# RQ015A Plan v6（复审后修订正文）— IPV 估计尝试状态与候选权重集中度回溯审计

状态：`POST_FINAL_REVIEW_REVISION / BUILD_WHILE_DENY`｜`formal_g1_eligible=false`｜`execution_authorized=false`  
日期：2026-07-30｜另存自 v5（v3/v4/v5 均保留为历史，不覆写）

本版在 v5 基础上吸收最终三路复审、复审后机器闸门修复、七轮独立健壮性审计、信任边界声明、schema v2 待决项的 PI 冻结判定、
...
本正文以以下实文件为依据：v5 计划正文、ledger schema v2、run spec v4、封存暴露登记、
预执行合同核验，以及 `scripts/rq015a/` 下当前实现。本文只写计划正文，不代写已知问题清单、
checksum manifest、PI 授权对象或任何执行回执。

v6 是最终复审后的修订正文，不是执行授权。任何后续运行必须另有 checksum-bound package、
...
当前分支 `rq015a-implementation`，HEAD `ab97822d`。从基线入版本控制到复审后修订，commit 链条为：
...
`f6fbfc0f`（审计 7 的 5 条）→ `ab97822d`（最终复审包组装）。

五个 RQ015A 测试文件当前合计 `243 passed`（起点 20）。复审时记录为 `229 passed`；
`229 -> 243` 是复审后修复 run spec 绑定、validate receipt 前置、checksum manifest 机器闸门后新增的覆盖。
验证命令形状：
...
- 重签清单后：`machine_verdict=PASS`，exit 0，`fixture_total_passed=243`。
...
随包提交的已知问题清单由指挥方另行成文，复审方必须与本 v6 同读。本文点名其中两条实质项：
...
v6 可作为复审后修订正文，但仍不授权执行。任何后续 package/run-spec 必须至少保留以下判定面：
