...
round2 output files listed at the end. Everything else is read-only.
...
- Manuscript: `manuscript_486382e.txt` (pdftotext of the reviewed PDF); LaTeX source at
...
2. `ROUND1_REVISION_PLAN.md` — the executable plan per §4 above.
...
## Round-2 additions to this charter

- The manuscript under review is `manuscript_41b819a.txt` (33 pages, post round-1 revision).
  Substitute that filename wherever the charter references the round-1 manuscript.
- Round-1 records are available for CROSS-REFERENCE ONLY at `../round1/` (AGGREGATION.md,
  ROUND1_REVISION_PLAN.md, ESCALATIONS.md, BACKLOG_draft.md). Use them to: (i) tag each round-2
  item as NEW / RESIDUAL (raised before, not yet fixed by design e.g. backlog/escalation) /
  REGRESSION (plausibly introduced by the round-1 revision); (ii) avoid re-proposing what is
  already backlogged or escalated — reference the existing ids instead.
- Explicit regression checks (verify in main.tex at commit 41b819a): the duplicated "References"
  heading D reports on p.26 (a manual \section*{References} above \bibliography prints twice
  under unsrt); the "empty section stubs on p.33" D reports (round-1 P27 was supposed to remove
  empty scaffolding — what remains?); the Fig. 4a rounding tension (21% vs the text's "about a
  fifth (21% and 20%)"); "9.845%" false precision vs "4.7%".
- Escalation continuity: E2/E5/E6/E7/E8/E10/E11 remain open with the PI. Do not re-litigate
  them; new facets fold into the existing escalation ids in your ESCALATIONS.md.
- Output files for this round: `AGGREGATION.md`, `ROUND2_REVISION_PLAN.md` (plan ids Q01...),
  `BACKLOG_delta.md` (only NEW backlog items, ids continuing B18...), `ESCALATIONS.md`
  (continuing E13... for genuinely new decisions) — all in THIS round2 directory.
