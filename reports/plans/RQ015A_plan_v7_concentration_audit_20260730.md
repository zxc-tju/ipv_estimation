# RQ015A Plan v7（复审条件闭合正文）— IPV 估计尝试状态与候选权重集中度回溯审计

状态：`POST_FINAL_REVIEW_REVISION / BUILD_WHILE_DENY`｜`formal_g1_eligible=false`｜`execution_authorized=false`  
日期：2026-07-30｜另存自 v6（v3/v4/v5/v6 均保留为历史，不覆写）

本版在 v6 基础上闭合三路最终复看条件：正文 provenance 不再硬写当前 HEAD，授权前增加 package commit 绑定，
execute 侧加固 validate receipt 前置校验。其余已通过复审的骨架继承 v6。RQ015A 的对象仍是 `attempt_status` 与连续
...
本正文以以下实文件为依据：v6 计划正文、ledger schema v2、run spec v5、封存暴露登记、
预执行合同核验，以及 `scripts/rq015a/` 下当前实现。本文只写计划正文，不代写已知问题清单、
checksum manifest、PI 授权对象或任何执行回执。

v7 是最终复审条件闭合后的修订正文，不是执行授权。当前正文的包身份不由正文里的 git HEAD 承载，
而由清单文件名 `reports/plans/RQ015A_plan_v7_checksums_20260730.sha256` 承载；该清单是内容寻址入口，
validate-only 必须逐行校验其成员集合与 SHA-256。正文若出现测试数，只能标为“截至清单签署时的实测值”；
机器权威始终是 validate-only 回执里的 `metadata.contract_fixtures.total_passed` / `fixture_total_passed`，
不是本正文。
...
本候选包身份由 `reports/plans/RQ015A_plan_v7_checksums_20260730.sha256` 与 run spec v5 的绑定共同承载。
...
随包提交的已知问题清单由指挥方另行成文，复审方必须与本 v7 同读。本文点名其中两条实质项：

- 同形字符列名仍可绕过 denylist：实测 `rаting`（西里尔 `а`）与 `scоre`（西里尔 `о`）均通过。
  这是已知剩余风险，不得在 v6 中改写成已闭合。复审方要求的限定是：同形字符问题披露充分，
...
v7 可作为复审后条件闭合正文，但仍不授权执行。任何后续 package/run-spec 必须至少保留以下判定面：

1. §10 的 40 条缺陷闭合叙述是否与当前实现、测试和已知问题清单一致；
2. §12 的信任边界是否足够明确，是否避免把 Python 对象伪造误列为无限 blocker；
3. §13 两条已知问题是否允许带入最终 package，或必须在执行授权前修复；
