...
from sociality_estimation.planning.utility import (
    get_central_vertices,
    bicycle_model,
    mass_point_model,
...
class Agent:
    ...
    def draw(self, fig_path='./outputs/'):
        ...
        plt.plot(cv_init_gs[:, 0], cv_init_gs[:, 1], '--', color='#006144')  # green
        plt.plot(cv_init_lt[:, 0], cv_init_lt[:, 1], '--', color='#0070C0')  # blue

        min_dis = 999

        for t in range(np.size(self.trj_solution, 0)):

            plt.plot(self.trj_solution[t, 0], self.trj_solution[t, 1], '*', color='#0070C0')
        ...
        plt.savefig(fig_path + 'final.svg')
    ...
    def solve_optimization(self, inter_track, *, solver_options=None):
        ...
        # 初始解
        u0 = np.concatenate([np.zeros([(track_len - 1), 1]),
                             np.zeros([(track_len - 1), 1])]).ravel()
...
if __name__ == '__main__':
    ...
    # 求解线性化博弈问题
    agent_lt.lp_ibr_interact(iter_limit=20, interactive=True, in_loop_draw=False, fig_path='./outputs/')
    time_all2 = time.perf_counter()
    print('LP overall time:', time_all2 - time_all1)

    # agent_lt.ibr_interact(iter_limit=20)
    # time_all3 = time.perf_counter()
    # print('non-linearized with constraints overall time:', time_all3 - time_all2)
    #
